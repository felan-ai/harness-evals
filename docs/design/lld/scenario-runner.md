...
1. The workspace is seeded once per `(test case, agent)` run, not once per step, so later steps observe earlier step changes. The seed is a copied local source/fixture, an exact Git checkout, or `seedPath` (default `/app`) extracted from the resolved Docker image. Git and image sources preserve `.git` in the run workspace.
...
7. Remove adapter-declared cleanup paths before the run finishes.
...
- The workspace is seeded once per test-case/agent run and shared by all steps in that run; it comes from a local source/fixture, an exact Git commit, or the image's `seedPath`.
